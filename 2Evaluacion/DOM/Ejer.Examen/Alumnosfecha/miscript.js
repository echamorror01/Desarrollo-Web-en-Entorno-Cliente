...
function añadirTarea() {
  ...
  // --- LÓGICA DE FECHAS (PUNTO CLAVE EXAMEN) ---
  let fechaLimite = new Date(); // Creamos la fecha de hoy

  // Queremos que la tarea caduque en 3 días
  let diaHoy = fechaLimite.getDate(); // Obtenemos el día (ej: 15)
  fechaLimite.setDate(diaHoy + 3); // Seteamos el día + 3 (ej: 18)

  // Opcional: ponerlo a última hora del día
...
function dibujarTareas() {
  ...
  tareasfiltradas.forEach(function (tarea, indice) {
    ...
    /* // --- LÓGICA DE COMPARACIÓN DE FECHAS (EXAMEN) ---
    let hoy = new Date();
    
    if (tarea.completada) {
      li.classList.add("completada");
      fechaLabel.textContent = `Terminada el: ${tarea.fechaCompletada}`;
      btnCompletar.textContent = "Deshacer";
    } else {
      // Si NO está completada, comprobamos si ha caducado
      if (hoy > tarea.limite) {
        li.style.backgroundColor = "#ffe6e6"; // Fondo rojizo si caduca
        fechaLabel.textContent = `CADUCADO (Límite: ${tarea.limite.toLocaleDateString()})`;
        fechaLabel.style.color = "red";
      } else {
        fechaLabel.textContent = `Límite: ${tarea.limite.toLocaleDateString()}`;
      }
